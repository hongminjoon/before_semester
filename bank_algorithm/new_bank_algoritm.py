...
# 클래스 선언
class User_Management: # User 관리 클래스
    def __init__(self):
        self.input_chance = 2
    ...
    def Print_User(self):
        try:
            with open('User.txt','r',encoding='utf8') as User:
                print(' ')
                print('----------------현재 유저 정보----------------')
                print('format. 이름, 아이디, 비밀번호, 계좌번호, 잔액')
                while True:
                    line = (User.readline()).strip('\n')
                    if not line: break
                    print(line)     
            print('-------------------------------------------\n')
    ...
    def CreateAN(self):   
        ...
        self.User_Data += account_number + ',' + self.initial_amount

        try: # 첫 유저이면 개행문자 없이 추가
            with open('User.txt','r',encoding = 'utf8') as User:
                pass
            with open('User.txt','a',encoding = 'utf8') as User:
                User.write('\n'+self.User_Data)
        except:
            with open('User.txt','a',encoding = 'utf8') as User:
                User.write(self.User_Data)
        return '0'
    ...
    def Delete_User(self):
        ...
        while True:
            ...
            if Delete_PW == Delete_line[2].strip('\n'): # 입력한 패스워드가 맞다면,
                ...
                with open('User.txt','w',encoding='utf8') as User:
                    for line in lines:
        ...
        try:
            with open('User.txt', 'r', encoding='utf8') as User:
                while True:
                    ...
                    if line[1].strip('\n') == Delete_ID:
                        break
        except: # Delete_ID 오입력시
            print('\n*****************************************')
            print('ID를 잘 못 입력하셨습니다. 초기화면으로 돌아갑니다.')
            print('*****************************************\n')
            return '0'
...
class Send_Money: # 랜덤 User에게 송금하는 클래스
    ...
    def send_money(self):    
        ...
        try:
            ...
            print('현재 유저가 없습니다. 유저를 생성한 후 실행해주십시요.')
            return '0'
        ...
        for i in range(randint(User_number, User_number+10)): # 최소 유저수 만큼 진행
            ...
            self.New_S_line = Old_S_line
            self.New_S_line[4] = New_S_balance

            # 송금자 New 정보를 입력
            if S_num == User_number: # 송금자가 마지막 위치에 있으면 Old_S_line 삭제 후 빈 한 줄이 남는 것을 처리
                with open('User.txt','a',encoding='utf8') as User:
                    User.write(','.join(self.New_S_line))
            else:
                with open('User.txt','a',encoding='utf8') as User:
                    User.write('\n' + ','.join(self.New_S_line))
            ...
            self.New_R_line = Old_R_line
            self.New_R_line[4] = New_R_Balance

            # 수금자 New 정보를 입력
            with open('User.txt','a',encoding='utf8') as User:
                User.write('\n' + ','.join(self.New_R_line))

            # 유저의 이름으로 계좌 송금 기록을 따로 보관
            try:
                with open(self.New_S_line[0]+'.txt','r',encoding='utf8') as User:
                    User.readline()
                with open(self.New_S_line[0]+'.txt','a',encoding='utf8') as User:
                    User.write(f'\n**출금 ( {self.New_R_line[0]} 님에게 ) : {self.amount}원**\n ===> 현재 잔액 : {self.New_S_line[4]}원\n')
            except:
                with open(self.New_S_line[0]+'.txt','w',encoding='utf8') as User:
                    User.write(f'**출금 ( {self.New_R_line[0]} 님에게 ) : {self.amount}원**\n ===> 현재 잔액 : {self.New_S_line[4]}원\n')

            # 유저의 이름으로 계좌 입금 기록을 따로 보관
            try:
                with open(self.New_R_line[0]+'.txt','r',encoding='utf8') as User:
                    User.readline()
                with open(self.New_R_line[0]+'.txt','a',encoding='utf8') as User:
                    User.write(f'\n**입금 ( {self.New_S_line[0]} 님으로부터 ) : {self.amount}원**\n ===> 현재 잔액 : {self.New_R_line[4]}원\n')
            except:
                with open(self.New_R_line[0]+'.txt','w',encoding='utf8') as User:
                    User.write(f'**입금 ( {self.New_S_line[0]} 님으로부터 ) : {self.amount}원**\n ===> 현재 잔액 : {self.New_R_line[4]}원\n')   
        return '0'

class Program_exe(User_Management, Send_Money): # Program 실행 클래스
    def __init__(self, service_no):
        User_Management.__init__(self)
        Send_Money.__init__(self)
        self.UserM = User_Management()
        self.SendM = Send_Money()
        self.service_no = service_no
    
    def execution(self):
        while True:
            if self.service_no == '1':
                self.UserM.Print_User()
                if self.UserM.CreateID() == '0':
                    self.service_no = '0'
                    continue
                if self.UserM.CreatePW() == '0':
                    self.service_no = '0'
                    continue
                if self.UserM.CreateAN() == '0':
                    self.service_no = '0'
                    continue

            if self.service_no == '2':
                self.UserM.Print_User()
                self.service_no = '0'
            
            if self.service_no == '3':
                if self.SendM.send_money() =='0':
                    self.service_no = '0'
            
            if self.service_no == '4':
                if self.UserM.Delete_User() == '0':
                    self.service_no = '0'
            
            if self.service_no != '0' and '1' and '2' and '3' and '4':
                print('\n***********************************')
                print('잘못된 값을 입력하셨습니다.')
                print('1, 2, 3, 4, Exit 중에 한가지를 입력해주십시요.')
                print('***********************************')
                self.service_no = '0'
            
            if self.service_no == '0':
                self.service_no = input('\n은행 서비스 프로그램 입니다.\
